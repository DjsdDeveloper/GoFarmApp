...
//using Chappsy2.Droid.DependencyInjection;
//using Chappsy2.Helpers;
//using Chappsy2.Interfaces;
//using Chappsy2.Models;
using Plugin.CurrentActivity;
using Plugin.Permissions;
using Plugin.Permissions.Abstractions;
using TuFarmaApp.Droid.DependencyInjection;
using TuFarmaApp.Helpers;
using TuFarmaApp.Interfaces;
using TuFarmaApp.Models;
using Xamarin.Forms;

[assembly: Dependency(typeof(MultiMediaPickerService))]
namespace TuFarmaApp.Droid.DependencyInjection
{
    public class MultiMediaPickerService : IMultiMediaPickerService
    {
            ...
            if (requestCode == MultiPickerResultCode)
            {
                if (resultCode == Result.Ok)
                {
                    ...
                    if (data != null)
                    {
                        ...
                        if (clipData != null)
                        {
                            //if (NImgCanAdd != 0 && clipData.ItemCount > NImgCanAdd)
                            //{

                            //    for (int i = 0; i < NImgCanAdd; i++)
                            //    {
                            ...
                            //    }
                            //    Toast.MakeText(Android.App.Application.Context, "Maximo 5 imagenes", ToastLength.Long).Show();
        ...
        public async Task<IList<MediaFile>> PickPhotosAsync(int Limit = 0)
        {
            NImgCanAdd = Limit;
