...
{
    public class TerminosPage : ContentPage
    {
        bool IsPhone;
        ...
        public TerminosPage()
        {
            ...
            #region Grid Terminos
            var GridTerminos = new Grid()
            {
                RowSpacing = 0,
                BackgroundColor = Color.FromHex("#eeeeee")
            };

            GridTerminos.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto });
            GridTerminos.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto });
            GridTerminos.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto });
            GridTerminos.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto });
            GridTerminos.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto });
            ...
            GridTerminos.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });
            GridTerminos.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(4, GridUnitType.Star) });
            GridTerminos.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });

            //GridLogin.SetBinding(Grid.IsVisibleProperty, nameof(LoginPageViewModel.IsVisibleLogin));

            GridTerminos.Children.Add(new Label
            {
                HorizontalOptions = LayoutOptions.Center,
            ...
            tapButtonSiguiente.SetBinding(TapGestureRecognizer.CommandProperty, nameof(TerminosPageViewModel.VolverCommand));
            frameButtonRegister.GestureRecognizers.Add(tapButtonSiguiente);

            GridTerminos.Children.Add(frameButtonRegister, 0, 3, 2, 3);
